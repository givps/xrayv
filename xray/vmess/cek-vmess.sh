...
data=( `cat /etc/xray/config/vmess.json | grep '^####' | cut -d ' ' -f 2`);
...
if [[ "$access" = "$ip" ]]; then
echo "$access" >> /tmp/ipvmess.txt
else
echo "$ip" >> /tmp/other.txt
fi
access2=$(cat /tmp/ipvmess.txt)
sed -i "/$access2/d" /tmp/other.txt > /dev/null 2>&1
done
access=$(cat /tmp/ipvmess.txt)
if [[ -z "$access" ]]; then
echo > /dev/null
else
access2=$(cat /tmp/ipvmess.txt | nl)
echo "user : $user";
echo "$access2";
echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
fi
